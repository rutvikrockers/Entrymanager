	...
	private class CheckInAsync extends AsyncTask<Void, Void, Object> {
		...
		protected void onPreExecute() {
			...
			Cursor crs = DatabaseHelper.getInstance(CameraTestActivity.this)
					.executeQuery("select max(id) as id from `tickets`");
			crs.moveToNext();
			this.ticket = crs.getInt(crs.getColumnIndex("id"));
			crs.moveToFirst();
		}
		...
		protected void onPostExecute(Object json) {
			if (json instanceof JSONObject) {
				try {
					...
					if(((JSONObject) json).getInt("status")==1){
						status.setImageResource(R.drawable.true_);
						...
						if (updateifExists(Integer.parseInt(((JSONObject) json).getString("attendee_id")))) {
...
//					if (((JSONObject) json).getJSONArray("attendee_id").get(0) instanceof String) {
//						title.setText(((JSONObject) json).getJSONArray(
//								"attendee_id").getString(0));
//						labelago.setText("");
//						labelno.setText("");
//						status.setImageResource(R.drawable.delete);
//					} else {
//						status.setImageResource(R.drawable.true_);
//						// title.setText(((JSONObject)json).getJSONArray("attendee_id").getString(0));
//						*//*
//						 * Log.e("test", ((JSONObject)
//						 * json).getJSONArray("attendee_id").getString(0));
//						 *//*
//						String str = new String("");
//						String ids = new String("");
//						DatabaseHelper
...
//												0, this.ticket));
//
//						for (int i = 0; i < ((JSONObject) json).getJSONArray(
//								"attendee_id").length(); i++) {
//							if (updateifExists(Integer
//									.parseInt(((JSONObject) json).getJSONArray(
//											"attendee_id").getString(i)))) {
//
//								Attendee attendee = fetchevent(Integer
//										.parseInt(((JSONObject) json)
//												.getJSONArray("attendee_id")
//												.getString(i)));
//								// Log.e("fs",attendee.first_name);
//								str += attendee.first_name + ", ";
//								ids += "#" + attendee.id + ",";
//							}
//						}
...
//						str += "(" + attendee.ticket_type + ")";
//						title.setText(str.toString());
//						// labelago.setText(R.string.last_scanned);
//						// Log.e("fs",str);
//						labelno.setText(ids);
//					}
					mCamera.setPreviewCallback(previewCb);
